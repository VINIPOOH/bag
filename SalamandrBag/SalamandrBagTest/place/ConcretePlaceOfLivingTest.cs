...
namespace TestSalamandrBag.place
{
    public class ConcretePlaceOfLivingTest
    {
        ...
        public void ShouldCommandVoiceToConcreteExistAnimal()
        {
            ...
            _mockAnimalOkkam.Setup(a => a.Name).Returns(animalName);
        ...
        public void ShouldCommandVoiceToAllAnimal()
        {
            ...
            _mockAnimalOkkam.Setup(a => a.Name).Returns(okamName);
            _mockAnimalOkkam.Setup(a => a.CommandVoice()).Returns(okamName);
            _mockAnimalKomuflor.Setup(a => a.Name).Returns(comuflorName);
        ...
        public void ShouldGetTotalFoodWeightPerDay()
        {
            ...
            _mockAnimalOkkam.Setup(a => a.WeightOfFoodPerDay).Returns(amountOfFoodForOkkam);
            _mockAnimalKomuflor.Setup(a => a.WeightOfFoodPerDay).Returns(amountOfFoodForKomuflor);
        ...
        public void ShouldGetAverageFoodWeightPerAnimal()
        {
            int amountOfFoodForOkkam = 100;
            int amountOfFoodForKomuflor = 150;
            float expectedAmountFood =
                (float) (amountOfFoodForOkkam + amountOfFoodForKomuflor) / _stubListOfAAnimals.Count;
            _mockAnimalOkkam.Setup(a => a.WeightOfFoodPerDay).Returns(amountOfFoodForOkkam);
            _mockAnimalKomuflor.Setup(a => a.WeightOfFoodPerDay).Returns(amountOfFoodForKomuflor);
